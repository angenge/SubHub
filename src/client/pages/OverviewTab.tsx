...
import { Activity, Radio, Layers, Shuffle, CheckCircle, Clock, Zap, ArrowUpRight, Shield, Loader2 } from 'lucide-react';
...
export const OverviewTab: React.FC<OverviewTabProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner / Welcome */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-sky-950/60 via-indigo-950/50 to-slate-900 border border-slate-800/80 p-6 md:p-8">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold bg-sky-600 hover:bg-sky-500 text-white shadow-lg shadow-sky-600/30 transition active:scale-95"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg shadow-indigo-600/30 transition active:scale-95"
            >
            ...
            {enableTcpPing && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-600/30 transition active:scale-95 disabled:opacity-50"
              >
                {isPingingAll ? (
                  <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div
          ...
          className="group cursor-pointer rounded-2xl bg-slate-900 border border-slate-800 p-5 hover:border-slate-700 transition hover:shadow-lg hover:shadow-sky-500/5"
        >
          <div className="flex items-center justify-between text-slate-400 mb-3">
            <span className="text-xs font-medium">有效订阅源</span>
            <div className="p-2 rounded-xl bg-sky-500/10 text-sky-400 group-hover:bg-sky-500 group-hover:text-white transition">
              <Radio className="w-4 h-4" />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-white tracking-tight">{stats.totalSubscriptions}</span>
            <span className="text-xs text-slate-500">个机场源</span>
          </div>
          <div className="mt-3 flex items-center text-xs text-sky-400 font-medium">
            <span>管理订阅与流量</span>
            <ArrowUpRight className="w-3.5 h-3.5 ml-0.5 group-hover:translate-x-0.5 transition" />
          </div>
        </div>
        ...
        <div
          ...
          className="group cursor-pointer rounded-2xl bg-slate-900 border border-slate-800 p-5 hover:border-slate-700 transition hover:shadow-lg hover:shadow-emerald-500/5"
        >
          <div className="flex items-center justify-between text-slate-400 mb-3">
            <span className="text-xs font-medium">总收录节点</span>
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-white tracking-tight">{stats.totalNodes}</span>
            <span className="text-xs text-emerald-400 font-medium">
              {enableTcpPing ? `(${stats.onlineNodes} 极速在线)` : '边缘全球分发就绪'}
            </span>
          </div>
        ...
        <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5">
          <div className="flex items-center justify-between text-slate-400 mb-3">
            <span className="text-xs font-medium">节点健康度</span>
            <button
              onClick={onPingAllNodes}
              disabled={isPingingAll || stats.totalNodes === 0}
              className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 text-xs font-medium border border-emerald-500/20 transition disabled:opacity-50"
              title="一键测速与检测健康度"
            >
              {isPingingAll ? (
              ...
              <span>{isPingingAll ? '测速中' : '一键测速'}</span>
            </button>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-white tracking-tight">{onlineRate}%</span>
            <span className="text-xs text-slate-400">存活率</span>
          </div>
        ...
        <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5">
          <div className="flex items-center justify-between text-slate-400 mb-3">
            <span className="text-xs font-medium">机场总流量已用</span>
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-sm text-slate-200">{sub.name}</span>
                    ...
                    <div className="flex items-center justify-between text-xs text-slate-400 mb-1.5 font-mono">
                      ...
                      <span>总量: {total > 0 ? formatBytes(total) : '无配额限制'}</span>
                    </div>
            ...
            <div className="flex flex-wrap gap-2">
              {stats.countryDistribution.length === 0 ? (
                ...
                stats.countryDistribution.map((item) => (
                  <div
                    key={item.country}
                    className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-950 border border-slate-800 text-xs text-slate-300"
                  >
                    <CountryBadge country={item.country} code={item.code} />
                    <span className="font-bold text-white ml-1">{item.count}</span>
                  </div>
                ))
              )}
            </div>
