...
import { Plus, RefreshCw, Trash2, Edit, Radio, ShieldAlert, ExternalLink, History, Power, PowerOff } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex-1 sm:flex-none justify-center flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700/80 transition active:scale-95"
            title="查看所有订阅的历史同步记录"
          >
          ...
          <button
            onClick={onAdd}
            className="flex-1 sm:flex-none justify-center flex items-center gap-1.5 sm:gap-2 px-3.5 sm:px-4 py-2 rounded-xl text-xs font-semibold bg-sky-600 hover:bg-sky-500 text-white shadow-lg shadow-sky-600/20 transition active:scale-95"
          >
      ...
      {subscriptions.length === 0 ? (
        <div className="text-center py-12 sm:py-16 rounded-2xl bg-slate-900/50 border border-slate-800 border-dashed space-y-3 p-4">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            点击上方「添加订阅」输入你的机场订阅链接，即可一键解析所有节点并进行聚合
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
          {subscriptions.map((sub) => {
            ...
            return (
              <div
                key={sub.id}
                className={`relative rounded-2xl bg-slate-900 border transition space-y-3 sm:space-y-4 shadow-sm p-4 sm:p-5 ${
                  isDisabled
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="space-y-1 min-w-0 flex-1">
                    ...
                    <p className="text-[11px] sm:text-xs font-mono text-slate-500 truncate">{sub.url}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {onToggleStatus && (
                      ...
                      >
                        {isDisabled ? <PowerOff className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <Power className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                      </button>
                    )}
                    <button
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-slate-400 hover:text-sky-400 hover:bg-slate-800 transition disabled:opacity-40 active:scale-95"
                      title="刷新此订阅"
                    >
                ...
                {/* Error Banner if any */}
                {sub.errorMessage && (
                  <div className="p-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-[11px] sm:text-xs flex items-start gap-2">
                    <ShieldAlert className="w-4 h-4 shrink-0 mt-0.5" />
                    <span className="break-all">{sub.errorMessage}</span>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-800/80 text-[11px] sm:text-xs text-slate-400 gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 sm:px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium transition shrink-0 active:scale-95"
                  >
